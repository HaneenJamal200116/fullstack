...
const Cart: React.FC<CartProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div
      className="fixed top-[88px] inset-0 flex justify-end z-50 "
      ...
      <div className="relative z-10 bg-white p-6  max-h-[500px] overflow-auto  shadow-lg w-[400px] right-15">
        ...
        <div className=" ">
          <div className="flex relative left-0">
            <p className="font-bold mr-3 mb-3">My Bag,</p>{" "}
            <p data-testid="cart-item-amount" className="mr-1">
              {itemCount}{" "}
            </p>
              ...
              >
                        ...
                        {attr.items.map((i: any) => (
                          <div
                            data-testid={`cart-item-attribute-${attr.name
                              ...
                              .replace(/\s+/g, "-")
                              .toLowerCase()}${
                              item.textAttr[attr.name] === i.value
                                ? "-selected"
                                : ""
                            }`}
                            className={`mr-2 text-xs mb-1 border p-1 ${
                              item.textAttr[attr.name] == i.value
                                ? "bg-black text-white"
                                : ""
                            }`}
                        ...
                        {attr.items.map((i: any) => (
                          <div
                            ...
                            data-testid={`product-attribute-${attr.name
                              ...
                              .replace(/\s+/g, "-")
                              .toLowerCase()}${
                              item.swatchAttr[attr.name] === i.value
                                ? "-selected"
                                : ""
                            }`}
                            className={`mr-2 mb-1 h-[17px] w-[17px] border p-1   text-sm
                                          ${
                                            item.swatchAttr[attr.name] ===
                                              i.value &&
                ...
                <div className="flex flex-col flex-1  h-[160px] justify-between items-center text-[24px] mt-2 mb-3">
                  <button
                    ...
                    className="cursor-pointer flex justify-center items-center text-[32px] border-1 border-[#1D1F22] h-6 w-6 hover:bg-gray-200"
                  >
                    +
                  </button>
          ...
          <div className="flex justify-between font-medium mt-8 font-[#1D1F22]">
            ...
            <p data-testid="cart-total">${sum.toFixed(2)}</p>
          </div>
