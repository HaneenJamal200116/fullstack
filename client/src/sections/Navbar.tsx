...
const Categories = () => {
    ...
    const NavItems =() => {
        return (
            ...
            {data.categories.map((category: { id: number; name: string }, index: number) => (
               <li 
               key={category.id} 
               onClick={() => setActiveTab(index)}
               data-testid={activeTab === index ? "active-category-link" : "category-link"}
               className={`h-14 px-6 cursor-pointer 
                   ${activeTab === index ? 'text-[#5ece7b] border-b-2 border-[#5ece7b] font-semibold' : 'text-black'}`}
           >
               {category.name}
           </li>
  ...
  return (
    <header className="fixed left-0 top-0 w-full max-h-20">
