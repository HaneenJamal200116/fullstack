    ...
    <section className='min-h-screen w-full  flex flex-col  relative top-[50px]'>
      ...
      <div className=" mt-5 ml-10">
        ...
        <ul className="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-5">
          {data.clothingProducts.map((product:any) => (
  
            <li  data-testid= {`product-${product.name.replace(/\s+/g, '-').toLowerCase()}`} 
          
              className="group  flex flex-col mb-6 hover:shadow-2xl hover:scale-105 transition-transform duration-300 ease-in-out "
              key={product.id}
            >
              <div>
                ...
                {product.instock && (   
                  ...
                  onClick={()=>{    console.log("Adding to cart:", product)
                    ...
                    product.textAttribute.forEach((attr: { name: string; items: { value: string }[] }) => {
                      defaultTextAttr[attr.name] = attr.items[0].value
                    })

                    
                    const defaultSwatchAttr: { [key: string]: string } = {}
                    product.swatchAttribute.forEach((attr: { name: string; items: { value: string }[] }) => {
                    ...
                    if (Array.isArray(prev)) {
                      ...
                      const existingProductIndex = flattenedPrev.findIndex(
                        (item: Details) =>
                          ...
                          JSON.stringify(item.textAttr) === JSON.stringify(defaultTextAttr)&&
                          JSON.stringify(item.swatchAttr) === JSON.stringify(defaultSwatchAttr)
                    ...
                    className="group-hover:flex hidden  hover:bg-green-500 cursor-pointer h-[52px] w-[52px] bg-[#5ece7b] rounded-full  relative bottom-13 flex items-center justify-center left-75">
                    <svg className="" fill="white" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 32.402 32">
              ...
              <div className="text-left ml-5 mb-5 mt-4">
                <div 
                    ...
                    onClick={() => navigate(`/clothes/${product.id}`)}>
                      {product.name}
                </div>
